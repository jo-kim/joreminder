...
### 2-1. 프로젝트 설정
- [ ] `frontend/`에 Next.js 프로젝트 생성 (TypeScript, App Router)
- [ ] `next.config.ts`에 API proxy 설정 (/api → localhost:8080)
- [ ] API 호출 유틸 함수 작성

### 2-2. 레이아웃
- [ ] 사이드바 + 메인 영역 2단 레이아웃 컴포넌트
- [ ] 사이드바에 목록 표시 (색상 불릿 + 이름 + 카운트)
- [ ] 목록 클릭 시 선택 상태 관리 (URL 또는 state)

### 2-3. 리마인더 표시
- [ ] 메인 영역에 선택된 목록의 리마인더 표시
- [ ] 목록 타이틀 (목록 색상, 볼드)
- [ ] 리마인더 행: 원형 체크박스 + 제목
- [ ] 완료 토글 클릭 시 API 호출 + UI 반영
...
### 3-1. 리마인더 생성
- [ ] "+ 새로운 미리 알림" 버튼 UI
- [ ] 클릭 시 새 행 추가 + 제목 입력 필드 포커스
- [ ] Enter 키로 저장 (POST API 호출)
- [ ] Escape 키로 취소

### 3-2. 리마인더 인라인 편집
- [ ] 리마인더 클릭 시 행 확장 → 제목 편집 모드
- [ ] 행 외부 클릭 시 저장 (PUT API 호출) + 편집 모드 닫힘

### 3-3. 리마인더 삭제
- [ ] 리마인더 hover 시 삭제 버튼 표시
- [ ] 삭제 확인 다이얼로그
- [ ] DELETE API 호출 + UI에서 제거
...
- [ ] "+ 목록 추가" 버튼 UI
- [ ] 모달 컴포넌트: 이름 입력 필드
- [ ] 12색 프리셋 원형 팔레트 색상 선택기
- [ ] 생성 완료 시 POST API 호출 + 사이드바 갱신

### 4-2. 목록 수정/삭제
- [ ] 목록 우클릭 시 컨텍스트 메뉴 (수정 / 삭제)
- [ ] 수정 시 생성과 동일한 모달 재사용 (PUT API)
- [ ] 삭제 시 확인 다이얼로그 + DELETE API 호출
- [ ] 기본 목록("미리 알림") 삭제 메뉴 비활성화
